                                       ...
                                       const int &maxX, const int &minY, const int &maxY, const int &N, const int &level)
{
    ...
    while(!bFinish)
    {
        ...
        // fixbug: when the mask is center-symetric, nodes count may keep the same after an iteration in early phase
        //if((int)lNodes.size()>=N || (int)lNodes.size()==prevSize)
        if((int)lNodes.size()>=N || ((int)lNodes.size()==prevSize && (int)lNodes.size()>=N/100))
        {
            bFinish = true;
        }
...
{ 
    ...
    for (int level = 0; level < nlevels; ++level)
    {
        ...
        keypoints.erase(std::remove_if(keypoints.begin(), keypoints.end(), [&](const cv::KeyPoint &keypoint)->bool{
            ...
            const cv::Point2f pt = keypoint.pt * scale;    
            CamModelGeneral::eFace face = CamModelGeneral::GetCamera()->FaceInCubemap(pt);
            if(face == CamModelGeneral::UNKNOWN_FACE)
                return true;
            if(pt.x<0 || (int)(pt.x+0.5f)>=width || pt.y<0 || (int)(pt.y+0.5f)>=height)
                return true;
            if(mask.at<uchar>( (int)(pt.y + 0.5f), (int)(pt.x + 0.5f) ) == 0)
                return true;
            else
            {
               keypoints_new.push_back(pt);
               return false;
            }
        }), keypoints.end());
