    ...
    mbFinishRequested(false), mbFinished(true), mbStopped(true), mbStopRequested(false)
{
    ...
    mImageWidth = fSettings["Camera.Iw"];
    mImageHeight = fSettings["Camera.Ih"];
...
void Viewer::Run()
{
    ...
    pangolin::CreateWindowAndBind("CubemapSLAM: Map Viewer",1024,768);
    ...
    cv::namedWindow("CubemapSLAM: Current Frame");
    ...
    while(1)
    {
        ...
        cv::imshow("CubemapSLAM: Current Frame",im);
        cv::waitKey(mT);
...
void Viewer::OutputTrackingSummary()
{
    mpFrameDrawer->OutputTrackingSummary();
}
